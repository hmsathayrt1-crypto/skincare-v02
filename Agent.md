...
- تمت تهيئة `rootNavigatorKey` في `app_router.dart` لتمكين استدعاء الحوارات والنوافذ المنبثقة من خارج شجرة الصفحات التقليدية.
- يتم تحديث وربط عنوان خادم الاتصال بالباك إند ديناميكياً عبر تعديل `ApiEndpoints.serverIpPort` وتعديل `DioClient().dio.options.baseUrl` بشكل فوري.

## 🧹 صيانة الذاكرة (Memory Hygiene)
### آخر تنظيف: 2026-06-15
